...
This project turns a LilyGO T-Display S3 Long into a USB password vault. It allows you to securely store passwords on the device and type them into a computer by emulating a USB keyboard. The device is unlocked with a PIN entered on the touch screen.
...
**This project is for educational purposes and should not be used for storing real, sensitive passwords without modification.**

The firmware uses a **hardcoded AES encryption key** located in `firmware/firmware.ino`. If you intend to use this for personal data, you **MUST** change this key to a unique, randomly generated 16-byte value. Failure to do so will render your stored passwords insecure.

To change it, open `firmware/firmware.ino` and modify this line:
```cpp
// Change these values to something random and secret!
...
2.  **(IMPORTANT) Change the Encryption Key:**
    *   In the VS Code file explorer, open the `firmware.ino` file.
    *   Find the `aes_key` line (around line 22) and change the hexadecimal values to your own secret key.
